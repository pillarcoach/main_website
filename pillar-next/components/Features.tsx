...
export default function Features() {
  return (
    <section className="border-t border-white/[0.07] overflow-hidden">
      <div className="px-6 md:px-10 lg:px-16 py-24 lg:py-36 lg:grid lg:grid-cols-2 lg:gap-16 lg:items-start">
        ...
        <div className="border-t border-white/[0.07]">
          {features.map((f, i) => (
            ...
            >
              <p className="text-[0.58rem] font-medium tracking-[0.22em] text-white/22 mb-3">{f.n}</p>
              <p className="text-[1.25rem] font-bold tracking-[0.06em] uppercase mb-3">{f.title}</p>
              <p className="text-[0.9375rem] leading-relaxed text-white/50">{f.body}</p>
            </motion.div>
          ))}
        </div>
